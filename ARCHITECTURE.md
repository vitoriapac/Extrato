...
O perfil de impacto reúne o valor já usado na prioridade com a evidência histórica, sem misturá-los nesta etapa. A origem do valor é `manual`, `estimated` (catálogo) ou `official` somente quando o peso da disciplina tem referência oficial. Incidência e peso observado em questões são `historical`, com confiança própria. A área estratégica e o editor do tópico exibem essas fontes e contagens; o motor de prioridade continua recebendo o mesmo `examImpact` de antes.
