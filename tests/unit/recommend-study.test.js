...
test('ordena recomendações pela fórmula explicável',()=>{
  ...
  assert.equal(result[0].id,'a');assert.ok(result[0].reasons.length);assert.equal(result[0].confidenceLabel,'Alta');
  assert.ok(result[0].contributions.examImpact>0);assert.equal(Object.values(result[0].contributions).reduce((sum,value)=>sum+value,0),result[0].score);
});
