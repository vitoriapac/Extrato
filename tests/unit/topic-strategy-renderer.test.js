...
import {buildTopicStrategyViewModel} from '../../src/features/topic-strategy/topic-strategy-view-model.js';
...
test('renderer apresenta impacto, campos e pré-requisitos elegíveis',()=>{
  ...
  const html=renderTopicStrategyEditor(buildTopicStrategyViewModel({subject,topic,topics}));
...
test('renderer escapa conteúdo e omite tópicos arquivados',()=>{
  const html=renderTopicStrategyEditor(buildTopicStrategyViewModel({subject:{id:'s1',name:'A'},topic:{id:'t1',name:'A'},topics:[{id:'t2',name:'<script>',subjectName:'B',topicArchived:true}]}));
