...
test('preserva identidade, versão e evidência da recomendação',()=>{
  ...
  const baseline={accuracy:58,questionVolume:32,measuredAt:'2026-09-01T12:00:00.000Z'};const feedback=recordRecommendationDecision(list,recommendation,{accepted:true,baseline,now:'2026-09-01T12:01:00.000Z',idGenerator:()=> 'feedback-1'});
  assert.equal(feedback.recommendationId,'rec-1');assert.equal(feedback.algorithmVersion,3);assert.equal(feedback.score,82);
  assert.deepEqual(feedback.baseline,baseline);assert.notEqual(feedback.baseline,baseline);assert.equal(feedback.outcome,null);
