...
import {buildStudyAction,recommendationActionKind,recommendationActionLabel,sameStudyActionTarget,STUDY_ACTION_SOURCES} from '../../src/application/recommendations/recommendation-action.js';
...
test('reconhece se a prioridade manteve o mesmo destino após recalcular',()=>{
  const before={subjectId:'s1',topicId:'t1',studyType:'study'},same={...before,recommendationId:'new-id',score:91},different={...before,studyType:'review'},missing=null;
  assert.equal(sameStudyActionTarget(before,same),true);assert.equal(sameStudyActionTarget(before,different),false);assert.equal(sameStudyActionTarget(before,missing),false);
});
