...
test('expõe estado de teste isolado com schema atual',async({page})=>{
  ...
  await expect.poll(()=>page.evaluate(()=>window.__EXTRATO_TEST__?.CURRENT_SCHEMA_VERSION)).toBe(22);
  const state=await page.evaluate(()=>window.__EXTRATO_TEST__.getState());
  expect(state.subjects.length).toBeGreaterThan(0);expect(state.schemaVersion).toBe(22);expect(state.examBlueprint.masteryTarget).toBe(80);
});
