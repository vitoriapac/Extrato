...
export function buildStudyCandidates({priorities=[],topics=[],retentions={},blueprint=[],sessions=[],today,examProximity=null}={}){
  ...
  const candidates=priorities.map(priority=>{
    ...
    const candidate={...priority,id:priority.topicId||priority.id,archived:Boolean(topic?.topicArchived||topic?.subjectArchived||topic?.archived),
      ...
      retention:retention?.available?retention.score:null,retentionRisk,retentionNeed:retentionRisk,reviewUrgency,
