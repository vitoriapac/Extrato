...
export function mergeExamImport(parsed,preview,decisions,{subjects=[],exams=[],examQuestions=[]}={}){
  ...
  const exam=createExam({...parsed.exam,id:examId,examTags:parsed.exam.examTags});
  if(existingIndex>=0)nextExams[existingIndex]={...nextExams[existingIndex],...exam};else nextExams.push(exam);
  let added=0,updated=0,ignored=0,createdTopics=0;
  for(const row of preview.rows){
    const decision=decisions?.[row.number];
    if(decision?.action==='ignore'){ignored++;continue}
