/* Arquivo gerado por build.ps1. Edite os modulos em src/, nao este bundle. */
(()=>{
'use strict';

/* source: src/state/schema.js */
const STORAGE_KEY='bb-premium-study-data';
const BACKUP_KEY=STORAGE_KEY+'-automatic-backup';
const BACKUP_INDEX_KEY=BACKUP_KEY+'-index';
const AUTOMATIC_BACKUP_SLOTS=5;
const CURRENT_SCHEMA_VERSION=8;
const MAX_BACKUP_FILE_SIZE=10*1024*1024;
const DB_NAME='extrato-estudos-db';
const DB_VERSION=1;
const STORE_NAME='app-state';

const STATUS_OPTIONS=['Não iniciado','Em andamento','Revisão','Concluído'];
const REVIEW_OPTIONS=['—','Revisão rápida','Revisão completa','Questões','Resumo/Mapa mental'];
const STATUS_CLASS={'Não iniciado':'st-nao','Em andamento':'st-and','Revisão':'st-rev','Concluído':'st-con'};
const TIPO_AGENDA_OPTIONS=['Revisão 24h','Revisão 3 dias','Revisão 7 dias','Revisão 14 dias','Revisão 15 dias','Revisão 30 dias','Revisão livre'];
const DIFFICULTY_OPTIONS=['Fácil','Médio','Difícil'];
const DIFFICULTY_WEIGHT={'Fácil':1,'Médio':2,'Difícil':3};
const DIFFICULTY_CLASS={'Fácil':'diff-facil','Médio':'diff-medio','Difícil':'diff-dificil'};
...
/* source: src/storage/repository.js */
...
function getDailyStudySummary(date){
  ...
  return {
    date,sessions,seconds,questions,correct,simulations,targetSeconds,goalPct,
...
document.getElementById('calFilterSubject').addEventListener('change', () => { renderCalendar(); renderMonthCalendar(); });
document.getElementById('calFilterStatus').addEventListener('change', () => { renderCalendar(); renderMonthCalendar(); });
...
function renderQuestionAnalytics(){
  ...
  const profile=getSubjectErrorProfile(performanceSubjectId);
  if(profile.totalErrors===0){
    profileEl.innerHTML='<div class="empty-state"><p>Nenhum erro registrado para esta disciplina.</p></div>';
  }else{
    const items=[...Object.entries(ERROR_CATEGORIES).map(([key,meta])=>({label:`${meta.icon} ${meta.label}`,value:profile.categories[key]})),{label:'Sem categoria',value:profile.uncategorized}];
    profileEl.innerHTML=`<div class="error-profile-grid">${items.map(item=>`<div class="error-profile-item"><span>${item.label}</span><strong>${item.value}</strong></div>`).join('')}</div>
      <div class="analytics-note">${profile.coverage}% dos ${profile.totalErrors} erros estão categorizados.</div>`;
  }
}
...
function diasSemEstudarDisciplina(subjectId){
  ...
  if(!last) return 21; // nunca estudada: trata como "bastante tempo sem estudar"
...
function computeAlertasInteligentes(){
  ...
  if(atrasadas > 0){
    alertas.push({ nivel:'alta', icon:'🔴', texto:`${atrasadas} revisão${atrasadas===1?'':'ões'} atrasada${atrasadas===1?'':'s'}` });
  }
  ...
  if(expectedFrac >= 0.5 && semanalFrac < expectedFrac - 0.15){
    ...
    alertas.push({ nivel:'media', icon:'🟠', texto:`Meta semanal ${gap}% abaixo do esperado pro dia da semana` });
  }
  ...
  if(difSemRevisao > 0){
    alertas.push({ nivel:'baixa', icon:'🟡', texto:`${difSemRevisao} tópico${difSemRevisao===1?'':'s'} difícil${difSemRevisao===1?'':'eis'} sem revisão agendada` });
  }

  const ritmo = computeRitmo();
  if(ritmo.status === 'ok' && ritmo.comparativo === 'no-prazo'){
    const folga = ritmo.daysToExam - ritmo.daysNeeded;
    alertas.push({ nivel:'ok', icon:'🟢', texto:`Ritmo atual permite terminar ${folga} dia${folga===1?'':'s'} antes da prova` });
  } else if(ritmo.status === 'ok' && ritmo.comparativo === 'atrasado'){
    const atraso = ritmo.daysNeeded - ritmo.daysToExam;
    alertas.push({ nivel:'alta', icon:'🔴', texto:`No ritmo atual você terminaria ${atraso} dia${atraso===1?'':'s'} depois da prova` });
  }
...
function renderAlertasInteligentes(){
  ...
  if(!container) return;
  const alertas = computeAlertasInteligentes();
...
const APPROVAL_WEIGHTS={simulados:0.25,acertos:0.10,edital:0.15,dominio:0.20,revisoes:0.10,tendencia:0.10,prazo:0.10};
...
function confiancaAprovacao(metrics){
  ...
  return {value,nivel:value>=0.70?'Alta':value>=0.35?'Média':'Baixa'};
}
function projectPerformance(metrics){
  ...
  if(sources.length===0) return {available:false,low:null,high:null,central:null,confidence:0,confidenceLabel:'Baixa',detail:'Registre questões, simulados e sessões para gerar uma faixa.'};
  ...
  const low=Math.max(0,Math.round(central-margin));
  const high=Math.min(100,Math.round(central+margin));
  return {
    available:true,low,high,central:Math.round(central),confidence,
    confidenceLabel:confidence>=0.70?'Alta':confidence>=0.35?'Média':'Baixa',
...
function approvalConsistenciaMetric(){
  const today=todayISO(),byDate=studySecondsByDate(state.studySessions);let applicable=0,achieved=0,studiedDays=0;
  for(let n=27;n>=0;n--){const date=addDays(today,-n),target=metaHoursForDate(date)*3600,studied=byDate[date]||0;if(studied)studiedDays++;if(target<=0)continue;applicable++;if(studied>=target)achieved++;}
  ...
  return {score:clampScore(50+(raw-50)*Math.max(.2,confidence)),confidence,available:studiedDays>0,raw,detail:achieved+' de '+applicable+' metas diárias atingidas nos últimos 28 dias'};
}
...
Object.assign(APPROVAL_WEIGHTS,{conhecimento:.35,retencao:.25,questoes:.20,simulados:.15,consistencia:.05});
...
function renderExamCountdown(){
  ...
  if(dias===null)fig.textContent='';else if(dias<0)fig.textContent='prova foi há '+Math.abs(dias)+' dia(s)';else if(dias===0)fig.textContent='a prova é hoje!';else fig.textContent='faltam '+dias+' dia'+(dias===1?'':'s')+' para a prova';
}
...
function completeUnifiedReview(id,origin){
  if(origin==='Agenda de Revisões'){const item=state.reviewAgenda.find(x=>x.id===id);if(!item||item.status==='Concluído')return;updateAgenda(id,'status','Concluído')}
...
const DELEGATED_ACTIONS=new Set([
  ...
  'editQuestion','editSimulation','editStudySession','gerarAgendaAutomatica','moveSubject','navigateKpi','renameSubject',
  'requestPermanentSubjectDelete','requestPermanentTopicDelete','resetAdaptiveReviewDate','restoreSubject','restoreTopic','saveQuestionEdit',
  'saveSimulationEdit','saveStudySessionEdit','selectSessionHistoryDate','startPlannedActivity','toggleBreakdown','toggleNotes',
  'toggleQuestionErrors','toggleSessionDay','toggleSubject','updateAgenda','updateBreakdownRow','updateCal','updateMeta',
...
if(TEST_MODE){
  ...
  window.__EXTRATO_TEST__={
    ...
    computeApprovalMetrics,indiceProntidao,computeStudyPriorities,topicRetentionScore,
