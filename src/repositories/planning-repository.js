export function createPlanningRepository({getState}={}){
  ...
  return Object.freeze({getStudyPlans:()=>plans(),getActiveStudyPlan:()=>[...plans()].reverse().sort((a,b)=>String(b.confirmedAt||'').localeCompare(String(a.confirmedAt||'')))[0]||null,saveStudyPlan:save(plans),getDailyPlans:()=>daily(),getDailyPlan:date=>daily().find(plan=>plan.date===date)||null,saveDailyPlan:save(daily),getAdjustments:()=>adjustments(),findAdjustment:id=>adjustments().find(item=>item.id===id)||null,saveAdjustment:save(adjustments)});
}
