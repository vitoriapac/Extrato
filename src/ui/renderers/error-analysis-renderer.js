export function renderErrorAnalysis(model,{toolbar='',escapeHtml=value=>String(value)}={}){
  ...
  const items=model.items.map(item=>'<div class="error-profile-item"><span class="error-profile-label">'+item.icon+' '+escapeHtml(item.label)+'</span><strong class="error-profile-value">'+item.value+'</strong><small class="error-profile-delta">'+(model.hasPrevious?(item.delta>=0?'+':'')+item.delta+' vs. período anterior':'Sem período anterior')+'</small></div>').join('');
