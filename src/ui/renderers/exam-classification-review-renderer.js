...
export function renderExamClassificationReview({exams=[],examQuestions=[],subjects=[],activeExamTags=[],filter='all',page=0}={}){
  ...
  const pageCount=Math.max(1,Math.ceil(rows.length/100)),currentPage=Math.max(0,Math.min(pageCount-1,page));
  const cards=rows.slice(currentPage*100,(currentPage+1)*100).map(row=>{
