...
export const DEMO_SCENARIO=Object.freeze({days:130,subjects:6,sessions:170,simulations:13,seed:'studytrack-demo-v3'});
...
export function generateDemoData({seed=DEMO_SCENARIO.seed,today}={}){
  ...
  for(let index=0;index<DEMO_SCENARIO.sessions;index++){
    ...
    if(type==='questions'){
      const resolved=22+Math.floor(random()*15),progress=(oldestAge-age)/oldestAge,subjectPenalty=entry.subject.id==='demo-subject-4'&&age<28?-10:0,rate=Math.max(42,Math.min(88,54+progress*24+subjectPenalty+(random()-.5)*10)),correct=Math.round(resolved*rate/100),errors=resolved-correct;
  ...
  state.progressHistory=Array.from({length:DEMO_SCENARIO.days},(_,index)=>({date:shiftDate(today,index-oldestAge),pct:Math.min(82,18+Math.floor(index*(64/oldestAge)))}));
