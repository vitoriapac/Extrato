...
export function resolveValidatedExamImpact(profile){
  ...
  const usable=['moderate','high'].includes(profile.confidence)&&profile.analyzedExamCount>=4&&profile.presencePercent!=null;
  if(!usable||['manual','official'].includes(profile.impactSourceType))return {value:baseline,usedHistory:false};
  const historical=Math.max(0,Math.min(100,Math.round(profile.presencePercent*.8+(profile.participationPercent??0)*.2)));
