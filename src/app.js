...
function renderStudyRecommendation(){
  ...
  container.querySelectorAll('.study-recommendation .recommendation-actions .btn:first-child').forEach((button,index)=>{button.textContent=`▶ ${visible[index]?.studyType==='questions'?'Resolver questões':visible[index]?.studyType==='review'?'Iniciar revisão':visible[index]?.blockedPrerequisites?.length?'Estudar pré-requisito':'Iniciar estudo'}`});
}
