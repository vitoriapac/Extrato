...
const normalizeExamSearch=value=>String(value||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLocaleLowerCase('pt-BR');
function syncExamSubjectCheckboxes(){document.querySelectorAll('[data-exam-subject]').forEach(input=>{const subject=selectedExamPreset().subjects.find(item=>item.id===input.dataset.examSubject),keys=(subject?.topics||[]).map(topic=>`${subject.id}:${topic.id}`),count=keys.filter(key=>examImportState.topicIds.has(key)).length;input.checked=keys.length>0&&count===keys.length;input.indeterminate=count>0&&count<keys.length})}
...
document.getElementById('examImportContent').addEventListener('click',event=>{const action=event.target.closest('[data-exam-select]')?.dataset.examSelect;if(!action)return;const preset=selectedExamPreset();examImportState.subjectIds.clear();examImportState.topicIds.clear();for(const subject of preset.subjects)for(const topic of subject.topics){const tags=topic.examTags||[],include=action==='all'||action==='bb'&&tags.includes(EXAM_TAGS.BB)||action==='caixa'&&tags.includes(EXAM_TAGS.CAIXA)||action==='caixa-ti'&&tags.includes(EXAM_TAGS.CAIXA_TI)||action==='common'&&isCommonTopic(topic,[EXAM_TAGS.BB,EXAM_TAGS.CAIXA]);if(include){examImportState.subjectIds.add(subject.id);examImportState.topicIds.add(`${subject.id}:${topic.id}`)}}renderExamImport()});
document.getElementById('examImportCancelBtn').addEventListener('click',closeExamImport);document.getElementById('examImportBackBtn').addEventListener('click',()=>{examImportState.step--;renderExamImport()});document.getElementById('examImportNextBtn').addEventListener('click',()=>{if(examImportState.step<3){examImportState.step++;renderExamImport();return}const preset=selectedExamPreset(),result=editalImportFacade.confirm(examImportState.subjectIds,examImportState.topicIds);applyPresetBlueprintDefaults(preset);persistAndRender();closeExamImport();showToast(`${pluralize(result.addedSubjects,'disciplina')}, ${pluralize(result.addedTopics,'tópico')} e ${pluralize(result.metadataUpdates,'vínculo')} atualizados.`)});
...
function computeAlertasInteligentes(){
  ...
  const dayOfWeek=new Date(today+'T00:00:00').getDay(),elapsed=dayOfWeek===0?7:dayOfWeek,expectedFrac=elapsed/7;
