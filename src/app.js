...
const historicalExamImportController=createHistoricalExamImportController({document,getState:()=>state,notify:showToast,onImported:result=>{
  state.subjects=result.subjects;state.exams=result.exams;state.examQuestions=result.examQuestions;
  studyPlanPreview=null;persistAndRender();
}});
historicalExamImportController.mount();
...
function updateExamClassificationReview(){
  ...
  content.innerHTML=renderExamClassificationReview({exams:state.exams,examQuestions:state.examQuestions,subjects:state.subjects,activeExamTags:state.examBlueprint.activeExamTags||[],filter:examClassificationFilter,page:examClassificationPage});
}
...
examClassificationOverlay?.addEventListener('click',event=>{
  ...
  const pageButton=event.target.closest('[data-review-page]');if(pageButton){examClassificationPage=Number(pageButton.dataset.reviewPage)||0;updateExamClassificationReview();document.getElementById('examClassificationFilter')?.focus();return}
...
function renderOverviewNextAction(availableMinutes){
  ...
  container.innerHTML=`${renderActiveStudyTimerNotice(item)}${outcome}<article class="card card--action overview-action-card ${inProgress?'is-in-progress':''}" data-study-action-source="${action.source}" data-study-action-id="${escapeAttr(action.id)}" data-activity-type="${action.activityType}"><div><h3>${escapeHtml(item.subjectName)} · ${escapeHtml(item.topicName)}</h3><p><strong>${minutes}</strong> · ${escapeHtml(label)} · prioridade ${action.priority??'—'}/100</p><p class="overview-action-reason">${escapeHtml(mastery)} · ${escapeHtml(action.evidence.label||model.evidenceLabel).toLowerCase()}</p>${inProgress?`<span class="active-study-status">Em andamento · ${sessionLabel}</span>`:''}</div><div class="overview-action-buttons">${actionButton}</div><details class="overview-action-explanation"><summary>Por que esta é a próxima ação?</summary><p><strong>Prova:</strong> ${escapeHtml(model.examExplanation)}</p><p><strong>Você:</strong> ${escapeHtml(model.personalExplanation)}</p><p><strong>Ação:</strong> ${escapeHtml(label)}. ${escapeHtml(reasons||'Selecionada pela prioridade atual, pelos pré-requisitos e pela disponibilidade de hoje.')}</p></details></article>`;
}
